...
export default function AuthProvider({ children }) {
  ...
  const updateCartState = async () => {
     ...
     if (savedUser) {
        ...
        try {
           const res = await axios.get(`${backendUrl}/cart/${user.email}`);
  ...
  const handleLogin = async (userData) => {
    ...
    if (guestCart.length > 0) {
       try {
          await axios.post(`${backendUrl}/cart/merge`, {
  ...
  const updateWishlistState = async () => {
     ...
     if (savedUser) {
        const user = JSON.parse(savedUser);
        try {
           const res = await axios.get(`${backendUrl}/wishlist/${user.email}`);
           items = res.data.items || [];
        } catch (err) {
  ...
  const addToWishlist = async (product) => {
     ...
     if (currentUser) {
        try {
           await axios.post(`${backendUrl}/wishlist`, {
              email: currentUser.email,
              productId: product.id,
              productData: product
           });
